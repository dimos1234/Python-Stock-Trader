...
class Portfolio():
    ...
    def add_position(self, symbol: str, asset_type: str, purchase_date: Optional[str], quantity: int = 0, purchase_price: float = 0.0) -> dict:
        ...
        self.positions[symbol]["purcahse_date"] = purchase_date
    ...
    def add_positions(self, positions: List[dict]) -> dict:
        if isinstance(positions, list):
            for position in positions:
                self.add_position(symbol=position["symbol"], asset_type=position["asset_type"], purchase_date=position.get("purchase_date", None), purchase_price=position.get("purchase_price", 0.0), quantity=position.get("quantity", 0))
